...
public class Conta {
	...
	public void deposita(double valor) {
		...
		if (valor < 0 ) {
			throw new IllegalArgumentException("Voc� tentou depositar um valor inv�lido");
		} else {
			this.saldo += valor;
		}
	}
	*/
	...
	public void deposita(double valor) {
		...
		if (valor <= 0 ) {
			throw new ValorInvalidoException("Valor inv�lido: R$ " + valor);
		} else {
			this.saldo += valor;
		}
	}
	...
	boolean transferePara(Conta destino, double valor) {
		...
		if (retirou == false) {
			...
			this.saquesDoDia--;
			destino.deposita(valor);		//..Faz o Dep�sito na Conta de Destino
			return true;
		}
	}	
